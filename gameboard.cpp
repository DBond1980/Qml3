...
bool GameBoard::move(int index)
{
    ...
    const Position elementPosition {getRowCol(static_cast<size_t>(index))};

    auto hiddenElementIterrator = std::find(m_rawBoard.begin(), m_rawBoard.end(), boardSize());

    const Position hiddenElementPosition {getRowCol(std::distance(m_rawBoard.begin(),
                                                           hiddenElementIterrator))};

    if(!isAdjacent(elementPosition, hiddenElementPosition))
    {
        return false;
    }
    //std::iter_swap(hiddenElementIterrator, elementPosition);
    std::swap(hiddenElementIterrator->value, m_rawBoard[static_cast<size_t>(index)].value);

    emit dataChanged(createIndex(0,0), createIndex(static_cast<int>(m_boardSize), 0));
